...
namespace xcore
{
	namespace xfilesystem
	{
		...
		public:
			...
			virtual void			write(const xbyte* buffer, s32 offset, s32 count) = 0;				///< When overridden in a derived class, writes a sequence of bytes to the current stream and advances the current position within this stream by the number of bytes written.
			...
			virtual xasync_result	beginRead(xbyte* buffer, s32 offset, s32 count, AsyncCallback callback) = 0;  		///< Begins an asynchronous read operation.
			virtual void			endRead(xasync_result& asyncResult) = 0;											///< Waits for the pending asynchronous read to complete.
			virtual xasync_result	beginWrite(const xbyte* buffer, s32 offset, s32 count, AsyncCallback callback) = 0;	///< Begins an asynchronous write operation.
			...
			virtual void			copyTo(xistream* dst, s32 count) = 0;								///< Reads all the bytes from the current stream and writes them to a destination stream, using a specified buffer size.
		};
