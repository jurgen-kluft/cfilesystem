...
namespace xcore
{
	class FileDevice_Mac_System : public xfiledevice
	{
		const char*			mDrivePath;
		xbool					mCanWrite;

	public:
								FileDevice_Mac_System(const char* pDrivePath, xbool boCanWrite) 
									: mDrivePath(pDrivePath)
									, mCanWrite(boCanWrite){ }
		virtual					~FileDevice_Mac_System()							{ }
	...
	xfiledevice*		x_CreateFileDeviceMac(const char* pDrivePath, xbool boCanWrite)
	{
		FileDevice_Mac_System* file_device = new FileDevice_Mac_System(pDrivePath, boCanWrite);
		return file_device;
	}

	void			x_DestroyFileDevicePS3(xfiledevice* device)
	{
		FileDevice_PS3_FileDevice_Mac_SystemSystem* file_device = (FileDevice_Mac_System*)device;
		delete file_device;
	}
	...
	bool FileDevice_Mac_System::openOrCreateFile(u32 nFileIndex, const char* szFilename, bool boRead, bool boWrite, u32& nFileHandle)
	{
		s32	nFlags;
		if(boWrite)
		{
	...
	bool FileDevice_Mac_System::hasFile(const char* szFilename) const
	{
		s32 nFlags;
		nFlags = CELL_FS_O_RDONLY;
		s32 hFile = INVALID_FILE_HANDLE;
	...
	bool FileDevice_Mac_System::openFile(const char* szFilename, bool boRead, bool boWrite, u32& outFileHandle) const
	{
		s32 nFlags;
		s32 hFile = INVALID_FILE_HANDLE;
		char systemDirBuffer[xfilepath::XFILEPATH_MAX ];
	...
	bool FileDevice_Mac_System::createFile(const char* szFilename, bool boRead, bool boWrite, u32& outFileHandle) const
	{
		s32 nFlags;
		if (boWrite)
		{
	...
	bool FileDevice_Mac_System::getLengthOfFile(u32 nFileHandle, u64& outLength) const
	{
		CellFsStat stats;
	...
	bool FileDevice_Mac_System::setLengthOfFile(u32 nFileHandle, u64 inLength) const
	{
		return false;
	}

	bool FileDevice_Mac_System::closeFile(u32 nFileHandle) const
	{
		s32 nResult = cellFsClose (nFileHandle);
		return nResult == CELL_OK;
	}

	bool FileDevice_Mac_System::deleteFile(const char* szFilename) const
	{
		char systemDirBuffer[xfilepath::XFILEPATH_MAX ];
		xcstring systemFile(systemDirBuffer, sizeof(systemDirBuffer));
		formatPath(szFilename,systemFile);
	...
	bool FileDevice_Mac_System::getDeviceInfo(u64& totalSpace, u64& freeSpace) const
	{
		return false;
	}

	bool FileDevice_Mac_System::readFile(u32 nFileHandle, u64 pos, void* buffer, u64 count, u64& outNumBytesRead) const
	{
		u64 newPos;
	...
	bool FileDevice_Mac_System::writeFile(u32 nFileHandle, u64 pos, const void* buffer, u64 count, u64& outNumBytesWritten) const
	{
		u64 newPos;
		if (seek(nFileHandle, __SEEK_ORIGIN, pos, newPos))
		{
	...
	bool FileDevice_Mac_System::seek(u32 nFileHandle, ESeekMode mode, u64 pos, u64& newPos) const
	{
		u64	nPos;
	...
	bool	FileDevice_Mac_System::seekOrigin(u32 nFileHandle, u64 pos, u64& newPos)
	{
		return seek(nFileHandle, __SEEK_ORIGIN, pos, newPos);
	}

	bool	FileDevice_Mac_System::seekCurrent(u32 nFileHandle, u64 pos, u64& newPos)
	{
		return seek(nFileHandle, __SEEK_CURRENT, pos, newPos);
	}

	bool	FileDevice_Mac_System::seekEnd(u32 nFileHandle, u64 pos, u64& newPos)
	{
		return seek(nFileHandle, __SEEK_END, pos, newPos);
	}

	bool FileDevice_Mac_System::moveFile(const char* szFilename, const char* szToFilename) const
	{
		if (!canWrite())
			return false;
	...
	bool FileDevice_Mac_System::copyFile(const char* szFilename, const char* szToFilename, bool boOverwrite) const
	{
		if (!canWrite())
			return false;
		...
		if (boOverwrite)
		{
			if (!const_cast<FileDevice_Mac_System* >(this)->openOrCreateFile((u32)0,systemFile_To.c_str(),true,true,nFileHandleTo))
				return false;
		}
		else
		{
	...
	bool FileDevice_Mac_System::setFilePos(u32 nFileHandle, u64& ioPos) const
	{
		if (canSeek())
		{
	...
	bool FileDevice_Mac_System::getFilePos(u32 nFileHandle, u64& outPos) const
	{
		if (canSeek())
		{
	...
	bool FileDevice_Mac_System::setFileTime(const char* szFilename, const xdatetime& creationTime, const xdatetime& lastAccessTime, const xdatetime& lastWriteTime) const
	{
		// invoke cellFsUtime
	...
	bool FileDevice_Mac_System::getFileTime(const char* szFilename, xdatetime& outCreationTime, xdatetime& outLastAccessTime, xdatetime& outLastWriteTime) const
	{
		CellFsStat stats;
		u32 nFileHandle;
		char systemDirBuffer[xfilepath::XFILEPATH_MAX ];
	...
	bool FileDevice_Mac_System::setFileAttr(const char* szFilename, const xattributes& attr) const
	{
		return false;
	}

	bool FileDevice_Mac_System::getFileAttr(const char* szFilename, xattributes& attr) const
	{
	...
	bool FileDevice_Mac_System::hasDir(const char* szDirPath) const
	{
		s32 hFile = INVALID_FILE_HANDLE;
		char systemDirBuffer[xfilepath::XFILEPATH_MAX ];
		xcstring systemFile(systemDirBuffer, sizeof(systemDirBuffer));
	...
	bool FileDevice_Mac_System::createDir(const char* szDirPath) const
	{
		char systemDirBuffer[xfilepath::XFILEPATH_MAX ];
		xcstring systemFile(systemDirBuffer, sizeof(systemDirBuffer));
		formatPath(szDirPath,systemFile);
	...
	bool FileDevice_Mac_System::copyDir(const char* szDirPath, const char* szToDirPath, bool boOverwrite) const
	{
	...
	bool FileDevice_Mac_System::deleteDir(const char* szDirPath) const
	{
	...
	bool FileDevice_Mac_System::moveDir(const char* szDirPath, const char* szToDirPath) const
	{
	...
	bool FileDevice_Mac_System::setDirTime(const char* szDirPath, const xdatetime& creationTime, const xdatetime& lastAccessTime, const xdatetime& lastWriteTime) const
	{
		// invoke cellFsUtime
		// This API is not supported
		return false;
	...
	bool FileDevice_Mac_System::getDirTime(const char* szDirPath, xdatetime& outCreationTime, xdatetime& outLastAccessTime, xdatetime& outLastWriteTime) const
	{
		CellFsStat stats;
		s32 nFileHandle;
		char systemDirBuffer[xfilepath::XFILEPATH_MAX ];
	...
	bool FileDevice_Mac_System::setDirAttr(const char* szDirPath, const xattributes& attr) const
	{
		return false;
	}

	bool FileDevice_Mac_System::getDirAttr(const char* szDirPath, xattributes& attr) const
	{
		return false;
	}

	bool FileDevice_Mac_System::enumerate(const char* szDirPath, bool boSearchSubDirectories, enumerate_delegate<xfileinfo>* file_enumerator, enumerate_delegate<xdirinfo>* dir_enumerator, s32 depth/* =0 */) const
	{
		char DirPathBuffer[CELL_FS_MAX_FS_PATH_LENGTH+2];
		char FileNameBuffer[CELL_FS_MAX_FS_PATH_LENGTH+2];
		xcstring DirPath(DirPathBuffer,sizeof(DirPathBuffer),szDirPath);
