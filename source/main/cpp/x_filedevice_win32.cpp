...
namespace xcore
{
	...
	bool FileDevice_PC_System::hasFile(const xfilepath& szFilename) const
	{
		...
		HANDLE nFileHandle = ::CreateFileW(LPCWSTR(szFilename.mRunes.m_str), fileMode, shareType, NULL, disposition, attrFlags, NULL);
	...
	bool FileDevice_PC_System::openFile(const xfilepath& szFilename, bool boRead, bool boWrite, void*& nFileHandle) const
	{
		...
		HANDLE handle = ::CreateFileW(LPCWSTR(szFilename.mRunes.m_str), fileMode, shareType, NULL, disposition, attrFlags, NULL);
		nFileHandle   = handle;
	...
	bool FileDevice_PC_System::writeFile(void* nFileHandle, u64 pos, const void* buffer, u64 count, u64& outNumBytesWritten) const
	{
		u64 newPos;
		if (seek(nFileHandle, __SEEK_ORIGIN, pos, newPos))
		{
	...
	bool FileDevice_PC_System::createDir(const xdirpath& szDirPath) const { return ::CreateDirectoryW(LPCWSTR(szDirPath.mRunes.m_str), NULL) != 0; }
	...
	static void changeDirPath(const xdirpath& szDirPath, const xdirpath& szToDirPath, const xdirinfo* szDirinfo, xdirpath& outDirPath)
	{
		xdirpath nDirpath_from(szDirPath);
		xdirpath nDirpath_to(szToDirPath);
		s32		 depth1 = nDirpath_from.getLevels();
		xdirpath dirpath, parent, child;
		szDirinfo->getDirpath(dirpath);
		dirpath.split(depth1, parent, child);
		nDirpath_to.getSubDir(child, outDirPath);
	}

	static void changeFilePath(const xdirpath& szDirPath, const xdirpath& szToDirPath, const xfileinfo* szFileinfo, xfilepath& outFilePath)
	{
		xdirpath nDir;
	...
	{
		xdirwalker walker;
		walker.mDirPath = szDirPath;
		
		utf16::runez<4> postfix;
		*postfix.m_end++ = '\\';
		*postfix.m_end++ = '*';
		*postfix.m_end   = '\0';

		utf16::concatenate(walker.mDirPath.mRunes, postfix, walker.mDirPath.mAlloc, 16);

		xheap nodeheap(allocator);
		xdirwalker::xnode* node = nodeheap.construct<xdirwalker::xnode>();

		node->mFindHandle = ::FindFirstFileW(LPCWSTR(walker.mDirPath.mRunes.m_str), &node->mFindData); // find the first file
		if (node->mFindHandle == INVALID_HANDLE_VALUE)
			return false;

		walker.mDirPath.mRunes.m_end -= 2;
		*walker.mDirPath.mRunes.m_end = '\0';

		bool bSearch = true;
		while (bSearch)
		{ // until we find an entry
			if (::FindNextFileW(node->mFindHandle, &node->mFindData))
			{
				if (sIsDots(node->mFindData.cFileName))
					continue;

				if ((node->mFindData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
				{
					utf16::runez<4> slash;
					*slash.m_end++   = '\\';
					*slash.m_end	 = '\0';
					utf16::concatenate(walker.mDirPath.mRunes, slash, walker.mDirPath.mAlloc, 16);

					utf16::runes dirname;
					dirname.m_str = (utf16::prune)node->mFindData.cFileName;
					dirname.m_end = dirname.m_str;
					dirname.m_eos = (utf16::prune)node->mFindData.cFileName[sizeof(node->mFindData.cFileName) - 1];
					while (dirname.m_end < dirname.m_eos && *dirname.m_end != '\0')
					{
						dirname.m_end++;
					}
					utf16::concatenate(walker.mDirPath.mRunes, dirname, walker.mDirPath.mAlloc, 16);
					
					slash.m_end	= slash.m_str;
					*slash.m_end++ = '\\';
					*slash.m_end++ = '*';
					*slash.m_end   = '\0';
					utf16::concatenate(walker.mDirPath.mRunes, slash, walker.mDirPath.mAlloc, 16);
					...
					xdirwalker::xnode* nextnode = nodeheap.construct<xdirwalker::xnode>();
					nextnode->mFindHandle		= ::FindFirstFileW(LPCWSTR(walker.mDirPath.mRunes.m_str), &nextnode->mFindData); // find the first file
					if (nextnode->mFindHandle != INVALID_HANDLE_VALUE)
					{
						// Link new node into linked-list
						// Just continue this while loop
					}
					else
					{
						walker.mDirPath.up();
				...
				else
				{
					...
					if (enumerator != NULL && fi != NULL)
					{
						bool terminate = (*enumerator)(depth, fi);
						bSearch = !terminate;
					}
					else
					{
			...
			else
			{
				...
				{
					// No more files here
					// Pop a node from the linked-list and continue with that
					// If no more nodes available then we are done
				}
				else
				{
	...
	bool FileDevice_PC_System::copyDir(const xdirpath& szDirPath, const xdirpath& szToDirPath, bool boOverwrite) const
	{
		if (boOverwrite)
		{
			if (::CreateDirectoryW(szToDirPath, NULL))
			{
				::RemoveDirectoryW(LPCWSTR(szToDirPath));
			}
			else
			{
		...
		enumerateCopyDir(szDirPath, &copy_enum, 0);
		...
		while (files_copy_enum.fileStack.pop(fileInfo))
		{
			...
			if (!::CopyFileW(fileInfo->getFullName().c_str_device(), copyFilePath_To.c_str_device(), false))
			{
				delete fileInfo;
				fileInfo = NULL;
				return false;
		...
		virtual bool operator()(s32 depth, const xfileinfo* finf, const xdirinfo* dinf)
		{
			if (finf != nullptr)
			{
				...
				if (dwFileAttributes & FILE_ATTRIBUTE_READONLY) // change read-only file mode
					::SetFileAttributesW((LPCTSTR)inf.getFullName().c_str_device(), dwFileAttributes & ~FILE_ATTRIBUTE_READONLY);
	...
	*/
	...
	bool FileDevice_PC_System::seekOrigin(u32 nFileHandle, u64 pos, u64& newPos) const { return seek(nFileHandle, __SEEK_ORIGIN, pos, newPos); }

	bool FileDevice_PC_System::seekCurrent(u32 nFileHandle, u64 pos, u64& newPos) const { return seek(nFileHandle, __SEEK_CURRENT, pos, newPos); }

	bool FileDevice_PC_System::seekEnd(u32 nFileHandle, u64 pos, u64& newPos) const { return seek(nFileHandle, __SEEK_END, pos, newPos); }
};
