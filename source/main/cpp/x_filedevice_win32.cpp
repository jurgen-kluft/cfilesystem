...
namespace xcore
{
    ...
    static void changeDirPath(const xdirpath& szDirPath, const xdirpath& szToDirPath, const xdirinfo* szDirinfo,
                              xdirpath& outDirPath)
    {
    ...
    static void changeFilePath(const xdirpath& szDirPath, const xdirpath& szToDirPath, const xfileinfo* szFileinfo,
                               xfilepath& outFilePath)
    {
        xdirpath nDir;
    ...
    struct xdirwalker
    {
        ...
        xdirwalker(xdirpath const& dirpath) : mDirPath(dirpath) {}

        bool enter_dir()
        {
            xnode* nextnode = mNodeHeap.construct<xdirwalker::xnode>();

            utf16::runez<4> wildcard;
            *postfix.m_end++ = '*';
            *postfix.m_end   = '\0';
            utf16::concatenate(mDirPath.mRunes, wildcard, mDirPath.mAlloc, 16);
        ...
        bool is_dots() const
        {
            if (sIsDots(mNode->mFindData.cFileName))
                continue;
        }

        bool is_dir() const { return ((node->mFindData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)); }
        bool next() { return ::FindNextFileW(node->mFindHandle, &node->mFindData); }
        ...
        void push_dir()
        {
            ...
            *slash.m_end   = '\0';
            ...
            utf16::runes dirname;
            dirname.m_str = (utf16::prune)mNode->mFindData.cFileName;
            dirname.m_end = dirname.m_str;
            dirname.m_eos = (utf16::prune)mNode->mFindData.cFileName[sizeof(mNode->mFindData.cFileName) - 1];
