...
namespace xcore
{
	...
	xfilepath::xfilepath(xfilesystem* parent, utf16::alloc* allocator, utf16::runes& str) : mFileSystem(parent), mAlloc(allocator), mRunes()
	{
		copy(str, mRunes, mAlloc, 16);
		fix_slashes(mRunes);
	}

	xfilepath::xfilepath(const xfilepath& filepath) : mFileSystem(filepath.mFileSystem), mAlloc(filepath.mAlloc), mRunes() { copy(filepath.mRunes, mRunes, mAlloc, 16); }
