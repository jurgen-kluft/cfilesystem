	...
	namespace xfilesystem
	{
		...
		void AsyncIOWorkerThread( u64 param )
		{
			while (xTRUE)
			{
				...
				{
					WaitForSingleObject(m_Sigal, INFINITE);
				}
			}
		}
		...
		void				Initialise ( u32 uAsyncQueueSize, xbool boEnableCache )
		{
			...
			m_AsyncIOThreadHandle = CreateThread(NULL, FS_ASYNC_WORKER_THREAD_STACK_SIZE, (LPTHREAD_START_ROUTINE)AsyncIOWorkerThread, NULL, 0, (DWORD*)&m_AsyncIOThreadID);
			SetThreadPriority(m_AsyncIOThreadHandle, THREAD_PRIORITY_HIGHEST);

			m_Sigal = CreateEvent(NULL, FALSE, FALSE, NULL);
		}	
...
// 			cellFsGetFreeSize(szPath, &uBlockSize, &uBlockCount);
		...
		void				CreateFileList( const char* szPath, xbool boRecursive )
		{
			...
			__private::ParseDir(szPath, boRecursive, m_uFileListLength, m_pszFileListData);
