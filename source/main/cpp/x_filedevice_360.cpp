...
namespace xcore
{
	...
	namespace xfilesystem
	{
		...
		public:
			FileDevice_360_System(const char* pDrivePath, bool boCanWrite)
		...
		xfiledevice*		x_CreateFileDevice360(const char* pDrivePath, bool boCanWrite)
		{
			FileDevice_360_System* file_device = new FileDevice_360_System(pDrivePath, boCanWrite);
			return file_device;
		}
		...
		bool FileDevice_360_System::getDeviceInfo(u64& totalSpace, u64& freeSpace) const
		{
		...
		bool FileDevice_360_System::hasFile(const char* szFilename) const
		{
		...
		bool FileDevice_360_System::openFile(const char* szFilename, bool boRead, bool boWrite, u32& nFileHandle) const
		{
			u32 shareType	= FILE_SHARE_READ;
		...
		bool FileDevice_360_System::writeFile(u32 nFileHandle, u64 pos, const void* buffer, u64 count, u64& outNumBytesWritten) const
		{
			u64 newPos;
			if (seek(nFileHandle, __SEEK_ORIGIN, pos, newPos))
			{
		...
		bool FileDevice_360_System::moveFile(const char* szFilename, const char* szToFilename) const
		{
			if (!canWrite())
				return false;
		...
		bool FileDevice_360_System::copyFile(const char* szFilename, const char* szToFilename, bool boOverwrite) const
		{
			if (!canWrite())
				return false;
		...
		bool FileDevice_360_System::closeFile(u32 nFileHandle) const
		{
		...
		bool FileDevice_360_System::deleteFile(const char* szFilename) const
		{
		...
		bool FileDevice_360_System::getFilePos(u32 nFileHandle, u64& outPos) const
		{
			if (canSeek())
			{
		...
		bool FileDevice_360_System::setFilePos(u32 nFileHandle, u64& ioPos) const
		{
			if (canSeek())
			{
		...
		bool FileDevice_360_System::setLengthOfFile(u32 nFileHandle, u64 inLength) const
		{
		...
		bool FileDevice_360_System::getLengthOfFile(u32 nFileHandle, u64& outLength) const
		{
		...
		bool FileDevice_360_System::setFileTime(const char* szFilename, const xdatetime& creationTime, const xdatetime& lastAccessTime, const xdatetime& lastWriteTime) const
		{
			u32 nFileHandle;
		...
		bool FileDevice_360_System::getFileTime(const char* szFilename, xdatetime& outCreationTime, xdatetime& outLastAccessTime, xdatetime& outLastWriteTime) const
		{
			u32 nFileHandle;
			if (openFile(szFilename, true, false, nFileHandle))
			{
		...
		bool FileDevice_360_System::setFileAttr(const char* szFilename, const xattributes& attr) const
		{
		...
		bool FileDevice_360_System::getFileAttr(const char* szFilename, xattributes& attr) const
		{
			DWORD dwFileAttributes = 0;
			...
			attr.setArchive ((dwFileAttributes & FILE_ATTRIBUTE_ARCHIVE )!=0);
			attr.setReadOnly((dwFileAttributes & FILE_ATTRIBUTE_READONLY)!=0);
			attr.setHidden  ((dwFileAttributes & FILE_ATTRIBUTE_HIDDEN  )!=0);
			attr.setSystem  ((dwFileAttributes & FILE_ATTRIBUTE_SYSTEM  )!=0);
		...
		bool FileDevice_360_System::hasDir(const char* szDirPath) const
		{
		...
		bool FileDevice_360_System::createDir(const char* szDirPath) const
		{
		...
		bool FileDevice_360_System::copyDir(const char* szDirPath, const char* szToDirPath, bool boOverwrite) const
		{
			if (boOverwrite)
			{
		...
		bool FileDevice_360_System::deleteDir(const char* szDirPath) const
		{
		...
		bool FileDevice_360_System::setDirTime(const char* szDirPath, const xdatetime& creationTime, const xdatetime& lastAccessTime, const xdatetime& lastWriteTime) const
		{
			HANDLE handle = sOpenDir(szDirPath);
			if (handle == INVALID_HANDLE_VALUE)
			{
		...
		bool FileDevice_360_System::getDirTime(const char* szDirPath, xdatetime& outCreationTime, xdatetime& outLastAccessTime, xdatetime& outLastWriteTime) const
		{
			HANDLE handle = sOpenDir(szDirPath);
			if (handle == INVALID_HANDLE_VALUE)
			{
		...
		bool FileDevice_360_System::setDirAttr(const char* szDirPath, const xattributes& attr) const
		{
			DWORD dwFileAttributes = 0;
			if (attr.isArchive())
				dwFileAttributes = dwFileAttributes | FILE_ATTRIBUTE_ARCHIVE;
		...
		bool FileDevice_360_System::getDirAttr(const char* szDirPath, xattributes& attr) const
		{
			DWORD dwFileAttributes = 0;
		...
		bool FileDevice_360_System::enumerate(const char* szDirPath, bool boSearchSubDirectories, enumerate_delegate<xfileinfo>* file_enumerator, enumerate_delegate<xdirinfo>* dir_enumerator, s32 depth) const
		{
			HANDLE hFind;  // file handle
		...
		bool FileDevice_360_System::seek(u32 nFileHandle, ESeekMode mode, u64 pos, u64& newPos) const
		{
			s32 hardwareMode = 0;
			switch(mode)
			{
		...
		bool	FileDevice_360_System::seekOrigin(u32 nFileHandle, u64 pos, u64& newPos) const
		{
			return seek(nFileHandle, __SEEK_ORIGIN, pos, newPos);
		}

		bool	FileDevice_360_System::seekCurrent(u32 nFileHandle, u64 pos, u64& newPos) const
		{
			return seek(nFileHandle, __SEEK_CURRENT, pos, newPos);
		}

		bool	FileDevice_360_System::seekEnd(u32 nFileHandle, u64 pos, u64& newPos) const
		{
